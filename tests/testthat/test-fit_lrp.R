...
test_that("fit_lrp reproduces the published chickpea optima", {
  ...
  for (i in seq_along(chickpea_cv_list)) {
    fit <- fit_lrp(chickpea_X, chickpea_cv_list[[i]])
...
test_that("the vector interface accepts positional and named arguments", {
  positional <- fit_lrp(chickpea_X, chickpea_CV1)
  named      <- fit_lrp(x = chickpea_X, cv = chickpea_CV1)
  expect_equal(positional$parameters, named$parameters)
})

test_that("the data-frame interface fits one model per trial", {
  res <- fit_lrp(chickpea_long, x = "x", cv = "cv", trial = "trial")
  ...
  alone <- fit_lrp(chickpea_X, chickpea_CV2)
...
test_that("method = 'ramp' runs and stays in a plausible range", {
  ramp <- fit_lrp(chickpea_X, chickpea_CV1, method = "ramp")
...
test_that("search_range restricts the breakpoint and is validated", {
  ...
  inside <- fit_lrp(chickpea_X, chickpea_CV1, search_range = c(5, 12))
  ...
  expect_warning(
    high <- fit_lrp(chickpea_X, chickpea_CV1, search_range = c(10, 18)),
    "edge of the search range"
  )
  expect_gte(unname(high$parameters["Breakpoint"]), 10)

  ## outside the data range, or inverted, is an error
  expect_error(fit_lrp(chickpea_X, chickpea_CV1, search_range = c(20, 40)),
               "within the data range")
  expect_error(fit_lrp(chickpea_X, chickpea_CV1, search_range = c(12, 5)),
               "lower < upper")
})
...
test_that("a non-decreasing series warns about the slope", {
  ...
  expect_warning(fit_lrp(chickpea_X, increasing), "slope is non-negative")
})
